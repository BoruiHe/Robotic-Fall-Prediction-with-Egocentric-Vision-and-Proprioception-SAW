...
class PoppyEnv(object):
    ...
    def __init__(self,
        ...
        self.steps = 0 # the number of steps, represents the length of the current environment
        self.num_hook = 0 # the number of times self.step_hook is executed
        ...
        if not wallpapers:
            ...
            texture_files = [f for f in texture_files if os.path.isfile(os.path.join(texture_path, f))]
            texture_files = random.sample(texture_files, 4) + ['sky.png']
        else:
            texture_files = wallpapers
        for i in range(5):
            ...
            pb.changeVisualShape(cubeid, -1, textureUniqueId= pb.loadTexture(os.path.join(texture_path, texture_files[i])))
    ...
    def step(self, action=None, sleep=False, hook=True):
