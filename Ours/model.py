...
class quickMLP(nn.Module):
    def __init__(self, ps, model_param):
        ...
        if model_param['dataset_name'] == 'vir_poppy':
            ...
            kl = 3
            strd = (1,2,2)
        self.conv = nn.Conv3d(3, 3, kl, stride=strd, dilation=dial, padding=(1,0,0))
        _, c, N, h, w = self.conv(torch.rand(1,3,model_param['N'],model_param['img_height'], model_param['img_width'])).shape
        print(N*h*w*c, model_param['N']*model_param['motion_dim'], ps*model_param['motion_dim_Traj'])
        in_features = N*h*w*c + model_param['N']*model_param['motion_dim'] + ps*model_param['motion_dim_Traj']
        mlp = []
        mlp.append(nn.Linear(in_features, 1000))
    ...
    def forward(self, motion_input_, traj_input_, frame_input_):
        frame_feat = self.conv(frame_input_)
        input_ = torch.cat((motion_input_, traj_input_, frame_feat.flatten(start_dim=1)), dim=1)
...
if __name__ == '__main__':
    ...
    from hp import hyperparameters_real
    x = torch.ones(64, 884736+600+250)
