...
from Ours.test_pos import test_siMLPe_step, test_Popeyes_step, test_traj_step
from Ours.dataset import SAWdataset
from Ours.model import siMLPe, siMLPe_, Popeyes, Traj
...
def train_siMLPe_step(input_joint_motion, gt, model, optimizer, nb_iter, dataset):
    ...
    gt = gt.reshape(b, output_seq_length, -1).cuda()
...
def train_siMLPe(hyperparameters, seed):
    ...
    while (nb_iter + 1) < hyperparameters['total_iterations']:
        for (input_joint_motion, future_joint_motion, _, _) in dataloader:
            loss, current_lr = train_siMLPe_step(input_joint_motion, future_joint_motion, model, optimizer, nb_iter, hyperparameters['dataset_name'])
            avg_lr += current_lr
            avg_train_loss += loss
...
def train_traj_step(input_joint_motion, model, optimizer, nb_iter, dataset) :
    ...
    # # (batch size, seq_len, motion dim)
    # b,n,c,d = input_joint_motion.shape
...
def svc_SP(hyperparameters, seed, siMLPe_weights, Popeyes_weights, prediction_span):
    ...
    X_train = sc.fit_transform(X_train)
    X_test = sc.transform(X_test)
    with open(os.path.join(hyperparameters['log_dir_path'], 'clf_SP_pos.txt'), 'a') as f:
        f.write(f'***** prediction span: {prediction_span} *****\n')
        clf = svm.LinearSVC(dual='auto', C=5e-3)
        clf.fit(X_train, y_train)
        predicted = clf.predict(X_test)
...
def svc_SPT(hyperparameters, seed, siMLPe_weights, Popeyes_weights, Traj_weights, prediction_span):
    ...
    X_train = sc.fit_transform(X_train)
    X_test = sc.transform(X_test)
    with open(os.path.join(hyperparameters['log_dir_path'], 'clf_SPT_pos.txt'), 'a') as f:
        f.write(f'***** prediction span: {prediction_span} *****\n')
        clf = svm.LinearSVC(dual='auto', C=5e-3)
        clf.fit(X_train, y_train)
        predicted = clf.predict(X_test)
