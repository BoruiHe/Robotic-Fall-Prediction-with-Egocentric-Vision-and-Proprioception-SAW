...
class SAWdataset(Dataset):
    def __init__(self, dataset_param, mode, output_frames, data_type='JA', for_visualization=False, ps=None, debug_path=None):
        ...
        if self.dataset == 'vir_poppy':
            self.data_path = os.path.join(os.getcwd(), 'data', 'virtual_poppy_scenes')
            with open(os.path.join(self.data_path, 'videos.pkl'), 'rb') as f:
            ...
            if self.mode == 'training':
                if os.path.isfile(os.path.join(path, 'intermediate_files', 'video_idx_splits.yml')):
                    ...
                    split_idx = self._split(dataset_param)
    ...
    def _split(self, dataset_param):
        ...
        # print('dataset size: {}, training set: {}, validation set: {} and testing set: {}'.format(
        #     len(video_dict['folder']), (splits_fall[0]+splits_walk[0])*k, (splits_fall[1]+splits_walk[1])*k, (splits_fall[2]+splits_walk[2])*k
        # ))
    ...
    def _get_idx_for_everything_VP(self, idx_reps, video_dict, path_to_VP):
        # print(f'For visualization? {self.for_visualization}')
        idx = []
        fall_idx, walk_idx = [], []
        ...
        if not self.for_visualization:
            ...
            idx.extend(walk_idx)
        idx.extend(fall_idx)
        # print(f'# of walk, fall and all sequences: {len(walk_idx), len(fall_idx), len(idx)}')
        return idx
...
if __name__ == '__main__':
    ...
    dataset = SAWdataset(
        ...
        debug_path= os.path.join(os.getcwd(), 'debugging', '608'),
        )
    print('VP:', len(dataset))
