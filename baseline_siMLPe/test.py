...
from baseline_siMLPe.model import siMLPe, Popeyes, Traj
from utils.logger import get_logger, print_and_log_info
from utils.miscellaneous import set_random_seeds, WeightedNormLoss, MSE, MSE_traj, MSE_siMLPe


def test_siMLPe_step(input_joint_motion, gt, model, mode):
    b, input_seq_length = input_joint_motion.shape[0], input_joint_motion.shape[1]
    output_seq_length = gt.shape[1]
    motion_input = input_joint_motion.reshape(b, input_seq_length, -1).cuda()
    motion_gt = gt.detach().cpu().reshape(b, output_seq_length, -1)
    with torch.no_grad():
        motion_input_ = motion_input.clone()
        output = model(motion_input_.cuda())
        motion_pred = output[:, :output_seq_length, :]
        motion_pred = motion_pred.detach().cpu()
        offset = motion_input[:, -1:].cpu()
        motion_pred += offset
    ...
    if mode == 'visualization': # return prediction and ground truth for plotting JA curves
        ...
        loss = WeightedNormLoss(motion_pred.cuda(), motion_gt.cuda(), 'sum')
...
def test_siMLPe(hyperparameters, seed, model_state_dict_pth, prediction_span):
    ...
    dataset = SAWdataset(hyperparameters, 'testing', False, data_type=hyperparameters['data_type'], ps=prediction_span)
    ...
    for (input_joint_motion, future_joint_motion, _, _) in dataloader:
        mean, std = test_siMLPe_step(input_joint_motion, future_joint_motion, model, 'testing')
        mse_ave += mean
        std_ave += std
